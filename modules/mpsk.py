...
class PSK:
    ...
    def bpsk(self, num_bits, EbN0_dBs):
    ...
    def qpsk(self, num_bits, EbN0_dBs):
    ...
    def PlotConstellation(self, num_bits, EbN0_range, M):
        ...
        if M == 2:
            ...
            for i in range(len(EbN0_range)):
                ...
                ber,ser,r = self.bpsk(num_bits, [EbN0_range[i]])
            ...
            for i in range(len(EbN0_range)):
                ...
                ber, ser, r_qpsk = self.qpsk(num_bits,[EbN0_range[i]])
                r_qpsk = r_qpsk[::1000]
                recebidos.append(r_qpsk)


            # Cria figura e subplots
            fig, axs = plt.subplots(1, 3, figsize=(14,4), sharex=False)

            for j in range(len(recebidos)):
                
                r_qpsk = np.array(recebidos[j])

                # calcula o ângulo de cada ponto em relação ao eixo real
                angles = np.angle(r_qpsk, deg=True)

                # separa os pontos em grupos com base no ângulo
                group1 = np.logical_and(angles >= -45, angles <= 45)
                group2 = np.logical_and(angles < -45, angles >= -135)
                group3 = np.logical_or(angles < -135, angles > 135)
                group4 = np.logical_and(angles > 45, angles <= 135)

                # define as cores para cada grupo
                colors = ['red', 'blue', 'red', 'blue']

                # plota os pontos em cores diferentes com base no grupo
                for i, group in enumerate([group1, group2, group3, group4]):
                    axs[j].scatter(r_qpsk.real[group], r_qpsk.imag[group], c=colors[i])
                    axs[j].set_ylabel('Q')
                    axs[j].set_xlabel('I')
                    axs[j].set_title('Eb/N0 = ' + str(EbN0_range[j]) + ' dB')
    ...
    def PlotBER_SER(self, num_bits, EbN0_dBs, M):
        ...
        if M == 2:
            ...
            BER, SER, r = self.bpsk(num_bits, EbN0_dBs)
        elif M == 4:
            teorico_ber = 0.5 * erfc(np.sqrt(10**(EbN0_dBs/10)))
            teorico_ser = 1 - (1 - 0.5 * erfc(np.sqrt(10**(EbN0_dBs/10))))**np.log2(M)
            BER, SER, r = self.qpsk(num_bits, EbN0_dBs)
        elif M == 8:
    ...
    def PlotWaveforms(self, bits_seq, fc, EbN0):
